...
describe('ShieldProofMempoolDatabase', () => {
  ...
  it('Should create collection indices', async () => {
    ...
    const indexFieldExists = indices.some((index) => {
      return (
        'key' in index && 'commitmentHash' in index.key && index.unique === true
      );
    });
  ...
  it('Should insert and get a valid shield proof mempool item', async () => {
    ...
    // Insert the item
    await db.insertShieldProof(shieldProofMempoolItem);

    // Fetch the item
    const fetchedItem = await db.getShieldProof(
      shieldProofMempoolItem.commitmentHash,
    );
    expect(fetchedItem).to.not.be.null;
    expect(fetchedItem).to.not.be.undefined;

    if (fetchedItem) {
      expect(fetchedItem.commitmentHash).to.equal(
        shieldProofMempoolItem.commitmentHash,
      );
      expect(fetchedItem.blindedCommitment).to.equal(
        shieldProofMempoolItem.blindedCommitment,
      );
      expect(fetchedItem.snarkProof).to.deep.equal(
        shieldProofMempoolItem.snarkProof,
      );
    }
  });
});
